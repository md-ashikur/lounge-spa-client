...
const Step2 = ({ bookingDetails, onBack, onNext }) => {
  ...
  const spaOptions = [
    { id: "None", name: "Aucune", price: 0, icon: remove },
    {
      ...
      price: 50,
      weekendPrice: 60, // Special price for weekends
      icon: extraHour,
    },
    {
      id: "massage",
...
Pour votre confort et votre tranquillité, des instructions claires et précises concernant le réchauffage des plats le nécessitant seront explicitement indiquée`,
  ...
  const calculateTotal = () => {
    ...
    if (
      ...
      let additionalHourPrice = isWeekend ? spaOptions.find(option => option.id === "1hr").weekendPrice : spaOptions.find(option => option.id === "1hr").price; // base price for each hour during weekends and weekdays
